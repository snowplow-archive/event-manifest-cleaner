...
JAR_FILE = "event-manifest-cleaner-0.1.0.jar"
...
def validate_input_path(s3_path):
    if not s3_path.startswith("s3://") and not s3_path.startswith("s3n://"):
        raise ValueError("Invalid enriched path {}. It should start with s3:// protocol".format(s3_path))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Snowplow Event Manifest Cleaner Spark job")

    subparsers = parser.add_subparsers()

    parser_run_emr = subparsers.add_parser(
        'run_emr',
        help="Run Event Manifest Cleaner on EMR cluster")
    parser_run_emr.add_argument(
        'run_folder',
        metavar='run-folder',
        type=str,
        help="S3 path to enriched events")
    parser_run_emr.add_argument(
        'storage_config',
    ...
    parser_run_emr.add_argument(
        ...
        metavar='time',
        type=str,
        help="Optional time for orphan records in DynamoDB")
